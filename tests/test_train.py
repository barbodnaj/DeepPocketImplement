...
def test_train_ddp_sim(cfg_train):
    """Simulate DDP (Distributed Data Parallel) on 2 mps processes."""
    HydraConfig().set_config(cfg_train)
    with open_dict(cfg_train):
...
def test_train_resume(tmp_path, cfg_train):
    ...
    assert metric_dict_1["train/mae"] < metric_dict_2["train/mae"]
    assert metric_dict_1["val/mae"] < metric_dict_2["val/mae"]
