...
class DeepPocketRSELitModule(LightningModule):
    ...
    def forward(self, x: torch.Tensor):
        encoder = self.encoderNet(x)
        decoder = self.decoderNet(encoder) 
        return decoder
...
class DeepPocketRSEEncoderLitModule(DeepPocketRSELitModule):
    def forward(self, x: torch.Tensor):
        encoder = self.encoderNet(x)
        return encoder
    def loudOnlyEncoder(self,checkPointPath:str):
        checkpoint = torch.load(checkPointPath)
        modifiedCheckpoint = checkpoint['model_state_dict']
        modifiedCheckpoint = {k.replace('decoder.', ''): v for k, v in modifiedCheckpoint.items() if 'encoder' in k}
        self.load_state_dict(modifiedCheckpoint)
        del self.decoderNet
    def freeze(self) -> None:
        return super().freeze()

if __name__ == "__main__":
    _ = DeepPocketRSELitModule(None, None, None, None)
    _ = DeepPocketRSEEncoderLitModule(None, None, None, None)
